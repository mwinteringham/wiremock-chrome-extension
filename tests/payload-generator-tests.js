...
describe('Generate payload function', function(){
  ...
  it('should generate a payload with headers', function(done) {
    ...
    dom.buildPayload('/thing', 'PATH', 'GET', 1, [], requestHeadersPayload, null, 200, [], '', function(payload){
  ...
  it('should generate a payload with response header', function(done) {
    var responseHeaderPayload = [
      {
        'key': 'key1',
        'value': 'value1'
      },{
    ...
    dom.buildPayload('/thing', 'PATH', 'GET', 1, [], [], null, 200, responseHeaderPayload, '', function(payload){
      expect(payload.response.headers['key1']).to.equal('value1');
      expect(payload.response.headers['key2']).to.equal('value2');
  ...
  it('should generate a response body payload when provided a response body', function(done){
    var generatedPayload = JSON.stringify({
      "total_results": 4,
      "array_result": [
        {
    ...
    dom.buildPayload('/thing', 'PATH', 'POST', 1, [], [], null, 200, [], generatedPayload, function(payload){
      expect(payload.response.body).to.equal(generatedPayload);
    });

    done();
  });

  it('should generate a xml response payload when provided a XML payload', function(done){
    var xmlPath = "<total_results>4</total_results>" +
                   "<array_result>" +
                   "<result>1</result>" +
                   "<result>2</result>" +
    ...
    dom.buildPayload('/thing', 'PATH', 'POST', 1, [], [], null, 200, [], xmlPath, function(payload){
      expect(payload.response.body).to.equal(xmlPath);
    });
